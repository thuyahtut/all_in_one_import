...
class ImportStore(models.Model):
    ...
    def check_sh_import_product_var(self):
        ...
        if record.sh_technical_name == 'sh_import_product_var':
            value = True
        return value

    import_type = fields.Selection(
        related="base_id.import_type", readonly=False, string="  Import File Type ", required=True)
    method = fields.Selection(related="base_id.method",
                                          readonly=False, string="Method", required=True)
    product_update_by = fields.Selection(
        related="base_id.product_update_by", readonly=False, string=" Product Variant Update By", required=True)
    is_create_m2m_record = fields.Boolean(
        related="base_id.is_create_m2m_record", readonly=False,string="Create a New Record for Dynamic M2M Field (if not exist)?")

    is_create_categ_id_record = fields.Boolean(
        related="base_id.is_create_categ_id_record", readonly=False,string="Create a New Record for Product Category Field (if not exist)?"
    )
    sh_import_product_var_boolean = fields.Boolean(
        "Import Variant Boolean", default=check_sh_import_product_var)
    sh_company_id_product_var = fields.Many2one(
        related="base_id.sh_company_id_product_var", readonly=False, string="Company", required=True)
    running_tmpl = fields.Char(string='Running Tmpl')
    total_done_tmpl = fields.Integer("Total Done Tmpl")

    def create_product_var_logs(self, counter, skipped_line_no,confirm_records):
    ...
    def perform_the_action(self, record):
        ...
        if record.base_id.sh_technical_name == 'sh_import_product_var':
            self.import_product_var(record)
        return res
    ...
    def import_product_var(self,record):
        self = record
            ...
            if self.import_type == 'csv' or self.import_type == 'excel':
                ...
                try:
                    ...
                    for row in range(self.count_start_from - 1, till):
                        try:
                            created_product_tmpl = product_tmpl_obj.sudo().search([('name','=',self.running_tmpl)],limit=1)
                            row = myreader[row]
                            count += 1
                            ...
                            if row[0] not in (None,
                                ...
                                if row[0] != self.running_tmpl:
                                    self.running_tmpl = row[0]
                                    ...
                                    if row[2].strip() not in ("",None) and row[3].strip() == 'FALSE':
                                        tmpl_vals.update({'purchase_ok': False})
                                    elif row[3].strip() not in ("",None) and row[3].strip() == 'TRUE':
                                        tmpl_vals.update({'purchase_ok': True})
                                    if row[4].strip() not in ("",False,None) and row[4].strip() == 'Service':
                                        tmpl_vals.update({'detailed_type': 'service'})
                                    elif row[4].strip() not in ("",False,None) and row[4].strip() == 'Storable Product' or row[4].strip() == 'Stockable Product':
                                        tmpl_vals.update({'detailed_type': 'product'})
                                    elif row[4].strip() not in ("",False,None) and row[4].strip() == 'Consumable':
...
#                                         created_product_tmpl.attribute_line_ids = False
                                    ...
                                    if has_variant == False and row[
                                            20] not in (None, ""):
                                        if created_product_tmpl and created_product_tmpl.product_variant_id and created_product_tmpl.detailed_type == 'product':
                                            stock_vals = {
                                                'product_tmpl_id':
                                                created_product_tmpl.id,
                                                'new_quantity':
                                    ...
                                    if row[14].strip() not in (
                                        ...
                                        if self.method == 'create':
                                            ...
                                            if created_product_tmpl.product_variant_ids:
                                                ...
                                                if product_varient.detailed_type == 'product' and row[
                                                        20] != '':
                                                    stock_vals = {
                                                        'product_tmpl_id':
                                                        created_product_tmpl.
                    ...
                    raise UserError(
                        _("Sorry, Your csv or excel file does not match with our format"
                          + ustr(e)))
                # if length_reader == self.total_done_tmpl:
                #     if not skipped_line_no:
                #         self.create_logs(
                #             self.total_done_tmpl, skipped_line_no,0)
                #         self.state = 'done'
                if counter > 1:
                ...
                #                 self.state = 'done'
                    if not skipped_line_no:
                        self.create_product_var_logs(
                            completed_records, skipped_line_no,imported_lines)
                        # if self.received_error:
                        #     self.state = 'partial_done'
                        # else:
                        self.state = 'done' if length_reader == self.total_done_tmpl else 'running'
                    else:
                        self.received_error = True
                        self.create_product_var_logs(
                            completed_records, skipped_line_no,imported_lines)
                        if self.on_error == 'break':
                            ...
                            if self.received_error:
                                ...
                                self.state = 'done' if length_reader == self.total_done_tmpl else None
