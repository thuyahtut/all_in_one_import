...
class ImportStore(models.Model):
    ...
    def check_sh_import_inv_with_payment(self):
        ...
        if record.sh_technical_name == 'sh_import_inv_with_payment':
            value = True
        return value

    sh_import_inv_with_payment_type = fields.Selection(related='base_id.sh_import_inv_with_payment_type',readonly=False, string="Import File Type")
    sh_import_inv_with_payment_product_by = fields.Selection(related='base_id.sh_import_inv_with_payment_product_by',readonly=False, string="Product By")
    sh_import_inv_with_payment_invoice_type = fields.Selection(related='base_id.sh_import_inv_with_payment_invoice_type',readonly=False, string="Invoicing Type")
    sh_import_inv_with_payment_is_validate = fields.Boolean(related='base_id.sh_import_inv_with_payment_is_validate',readonly=False,string="Auto Post?")
    sh_import_inv_with_payment_is_payment = fields.Boolean(related='base_id.sh_import_inv_with_payment_is_payment',readonly=False,string="Auto Payment?")
    sh_import_inv_with_payment_payment_option = fields.Selection(related='base_id.sh_import_inv_with_payment_payment_option',readonly=False, string="Payment Type")
    sh_import_inv_with_payment_write_off_account_id = fields.Many2one(
        'account.account', string='Post Difference In',related='base_id.sh_import_inv_with_payment_write_off_account_id',readonly=False)
    sh_import_inv_with_payment_account_option = fields.Selection(related='base_id.sh_import_inv_with_payment_account_option',readonly=False, string="Account")

    sh_import_inv_with_payment_inv_no_type = fields.Selection(related='base_id.sh_import_inv_with_payment_inv_no_type',readonly=False, string="Number", required=True)

    sh_import_inv_with_payment_partner_by = fields.Selection(related='base_id.sh_import_inv_with_payment_partner_by',readonly=False, string="Customer By")
    sh_import_inv_with_payment_boolean = fields.Boolean(
        "Import Invoice With Payment Boolean", default=check_sh_import_inv_with_payment)
    sh_company_id_inv_with_payment = fields.Many2one(
        related="base_id.sh_company_id_inv_with_payment", readonly=False, string="Company", required=True)
    running_inv_pmt = fields.Char(string='Running Invoice With Payment')
    total_done_inv_pmt = fields.Integer("Total Done Invoice With Payment")

    def create_inv_payment_logs(self, counter, skipped_line_no,confirm_records):
    ...
    def perform_the_action(self, record):
        ...
        if record.base_id.sh_technical_name == 'sh_import_inv_with_payment':
            self.import_inv_with_payment(record)
        return res

    def import_inv_with_payment(self,record):
        self = record
        inv_obj = self.env['account.move']
        # # perform import Invoice
        if self and self.sh_file:
            if self.sh_import_inv_with_payment_type == 'csv' or self.sh_import_inv_with_payment_type == 'excel':
                counter = 1
                skipped_line_no = {}
                count=0
                # running_inv = None
                ...
                imported_lines = 0
                try:
                    if self.sh_import_inv_with_payment_type == 'csv':
                    ...
                    elif self.sh_import_inv_with_payment_type == 'excel':
                    ...
                    self.total_done_inv_pmt = till    
                    for row in range(self.count_start_from - 1, till):
                        try:
                            created_inv = inv_obj.search([('name','=',self.running_inv_pmt)],limit=1)
                            row = myreader[row]
                            count += 1
                            ...
                            if row[0] not in (None, "") and row[4] not in (None, ""):
                                ...
                                if row[0] != self.running_inv_pmt:
                                    if self.sh_import_inv_with_payment_is_payment:
                                        if self.sh_import_inv_with_payment_payment_option == 'partial':
                                            # payment
                                            if created_inv and len(journals) > 0 and len(payment_amount) > 0 and len(journals) == len(payment_amount):
                                                ...
                                                payment_amount = []
                                                payment_date = False
                                        elif self.sh_import_inv_with_payment_payment_option == 'write_off':
                                            if created_inv:
                                                list_pay_dic = []
                                                payment_dic = {
                                                    "payment_date": payment_date or fields.Date.today(),
                                                    ...
                                                    'writeoff_account_id': self.sh_import_inv_with_payment_write_off_account_id.id,
                                    ...
                                    if row[1] not in (None, ""):
                                        ...
                                        if self.sh_import_inv_with_payment_partner_by == 'name':
                                            domain += [('name', '=', row[1])]
                                        if self.sh_import_inv_with_payment_partner_by == 'ref':
                                            domain += [('ref', '=', row[1])]
                                        if self.sh_import_inv_with_payment_partner_by == 'id':
                                            domain += [('id', '=', row[1])]
                                    ...
                                    if self.sh_import_inv_with_payment_inv_no_type == 'as_per_sheet':
                                        inv_vals.update({"name": row[0]})
                                    # created_inv = False
                                    if self.sh_import_inv_with_payment_invoice_type == 'inv':
                                        inv_vals.update(
                                            {"move_type": "out_invoice"})
                                        created_inv = inv_obj.with_context(
                                            default_move_type='out_invoice').create(inv_vals)
                                    elif self.sh_import_inv_with_payment_invoice_type == 'bill':
                                        inv_vals.update(
                                            {"move_type": "in_invoice"})
                                        created_inv = inv_obj.with_context(
                                            default_move_type='in_invoice').create(inv_vals)
                                    elif self.sh_import_inv_with_payment_invoice_type == 'ccn':
                                        inv_vals.update(
                                            {"move_type": "out_refund"})
                                        created_inv = inv_obj.with_context(
                                            default_move_type='out_refund').create(inv_vals)
                                    elif self.sh_import_inv_with_payment_invoice_type == 'vcn':
                                        inv_vals.update(
                                            {"move_type": "in_refund"})
                                        created_inv = inv_obj.with_context(
                                ...
                                if created_inv:
                                    field_nm = 'name'
                                    if self.sh_import_inv_with_payment_product_by == 'name':
                                        field_nm = 'name'
                                    elif self.sh_import_inv_with_payment_product_by == 'int_ref':
                                        field_nm = 'default_code'
                                    elif self.sh_import_inv_with_payment_product_by == 'barcode':
                                        field_nm = 'barcode'
                                    ...
                                    if search_product:
                                        ...
                                        account = False
                                        if self.sh_import_inv_with_payment_account_option == 'default':
                                        ...
                                        elif self.sh_import_inv_with_payment_account_option == 'sheet':
...
#                     validate invoice
                    if created_inv_list_for_validate and self.sh_import_inv_with_payment_is_validate:
                        invoices = inv_obj.search(
                    ...
                    raise UserError(
                        ...
                        + ustr(e)))
                # if length_reader == self.total_done_inv_pmt:
                    # if not skipped_line_no:
                    #     self.create_inv_payment_logs(
                    #         self.total_done_inv_pmt, skipped_line_no,0)
                    #     self.state = 'done'
                ...
                #                 self.state = 'done'
                    if not skipped_line_no:
                        ...
                        if self.received_error:
                            ...
                            self.state = 'done' if length_reader == self.total_done_inv_pmt else 'running'
                    else:
                        self.received_error = True
                        self.create_inv_payment_logs(
                            completed_records, skipped_line_no,imported_lines)
                        if self.on_error == 'break':
                            ...
                            if self.received_error:
                                ...
                                self.state = 'done' if length_reader == self.total_done_inv_pmt else None
