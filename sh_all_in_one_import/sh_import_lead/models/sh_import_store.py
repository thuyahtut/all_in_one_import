...
class ImportStore(models.Model):
    ...
    def check_sh_import_lead(self):
        ...
        if record.sh_technical_name == 'sh_import_lead':
            value = True
        return value

    sh_import_lead_file_type = fields.Selection(related='base_id.sh_import_lead_file_type',readonly=False, string="Import File Type", required=True)
    sh_import_lead_type = fields.Selection(related='base_id.sh_import_lead_type',readonly=False, string="Import Type")
    sh_import_lead_boolean = fields.Boolean(
        "Import Lead Boolean", default=check_sh_import_lead)
    sh_company_id_lead = fields.Many2one(
        related="base_id.sh_company_id_lead", readonly=False, string="Company", required=True)
    ...
    def perform_the_action(self, record):
        ...
        if record.base_id.sh_technical_name == 'sh_import_lead':
            self.import_lead(record)
        return res

    def import_lead(self,record):
        self = record
        ...
        if self and self.sh_file:
            ...
            if self.sh_import_lead_file_type == 'csv' or self.sh_import_lead_file_type == 'excel':
                counter = 1
                skipped_line_no = {}
                count=0
                row_field_dic = {}
                row_field_error_dic = {}
                try:
                    if self.sh_import_lead_file_type == 'csv':
                    ...
                    elif self.sh_import_lead_file_type == 'excel':
