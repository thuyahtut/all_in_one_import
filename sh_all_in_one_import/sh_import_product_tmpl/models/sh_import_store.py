...
class ImportStore(models.Model):
    ...
    def check_sh_import_product_temp(self):
        ...
        if record.sh_technical_name == 'sh_import_product_tmpl':
            value = True
        return value

    sh_import_prod_temp_import_type = fields.Selection(
        related="base_id.sh_import_prod_temp_import_type", readonly=False, string="  Import File Type ", required=True)
    sh_import_prod_temp_method = fields.Selection(related="base_id.sh_import_prod_temp_method",
                                          readonly=False, string="Method", required=True)
    sh_import_prod_temp_product_update_by = fields.Selection(
        related="base_id.sh_import_prod_temp_product_update_by", readonly=False, string=" Product Update By", required=True)
    sh_import_product_temp_boolean = fields.Boolean(
        "Import Product Template Boolean", default=check_sh_import_product_temp)
    sh_company_id_product_temp = fields.Many2one(
        related="base_id.sh_company_id_product_temp", readonly=False, string="Company", required=True)
    total_done_product_temp = fields.Integer("Total Done Product Template ")
    ...
    def perform_the_action(self, record):
        ...
        if record.base_id.sh_technical_name == 'sh_import_product_tmpl':
            self.import_product_temp(record)
        return res
    ...
    def import_product_temp(self,record):
        self = record
        ...
        if self and self.sh_file:
            ...
            # For CSV
            if self.sh_import_prod_temp_import_type == 'csv' or self.sh_import_prod_temp_import_type == 'excel':
                counter = 1
                skipped_line_no = {}
                row_field_dic = {}
                row_field_error_dic = {}
                count=0
                try:
                    if self.sh_import_prod_temp_import_type == 'csv':
                    ...
                    elif self.sh_import_prod_temp_import_type == 'excel':
                    ...
                    self.total_done_product_temp = till
                    ...
                    for row in range(self.count_start_from - 1, till):
                        try:
                            ...
                            if row[0].strip() not in (None, ""):
                                ...
                                if self.sh_import_prod_temp_method == 'create':
                                    if row[6].strip() in (None, ""):
                                        ...
                                        if search_product_tmpl:
                                            ...
                                            created_product_tmpl = product_tmpl_obj.create(
                                                vals)
                                            counter = counter + 1
                                elif self.sh_import_prod_temp_method == 'write' and self.sh_import_prod_temp_product_update_by == 'barcode':
                                    if row[6].strip() in (None, ""):
                                        created_product_tmpl = product_tmpl_obj.create(
                                            vals)
                                        counter = counter + 1
                                        ...
                                        if search_product_tmpl:
                                            ...
                                            created_product_tmpl = product_tmpl_obj.create(
                                                vals)
                                            counter = counter + 1
                                elif self.sh_import_prod_temp_method == 'write' and self.sh_import_prod_temp_product_update_by == 'int_ref':
                                    search_product_tmpl = product_tmpl_obj.search(
                ...
                if counter > 1:                
                    ...
                    if not skipped_line_no:
                        ...
                        # self.state = 'done'
                        self.state = 'done' if length_reader == self.total_done_product_temp else 'running'
                    else:
                        ...
                        if self.on_error == 'break':
                            ...
                            if self.received_error:
                                ...
                                # self.state = 'done'
                                self.state = 'done' if length_reader == self.total_done_product_temp else None
