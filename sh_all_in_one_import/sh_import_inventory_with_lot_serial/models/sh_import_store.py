...
class ImportStore(models.Model):
    ...
    def check_sh_import_with_lot_serial(self):
        ...
        if record.sh_technical_name == 'sh_import_inventory_with_lot_serial':
            value = True
        return value

    sh_import_inventory_with_lot_serial_type = fields.Selection(related='base_id.sh_import_inventory_with_lot_serial_type',readonly=False,string="Import File Type", required=True)

    sh_import_inventory_with_lot_serial_product_by = fields.Selection(related='base_id.sh_import_inventory_with_lot_serial_product_by',readonly=False, string="Product By", required=True)

    sh_import_inventory_with_lot_serial_location_id = fields.Many2one("stock.location", string="Location",
                                  related='base_id.sh_import_inventory_with_lot_serial_location_id',readonly=False)
    sh_import_inventory_with_lot_serial_name = fields.Char(string="Inventory Reference", related='base_id.sh_import_inventory_with_lot_serial_name',readonly=False)

    sh_import_inventory_with_lot_serial_is_create_lot = fields.Boolean(related='base_id.sh_import_inventory_with_lot_serial_is_create_lot',readonly=False,string="Create Lot/Serial Number if never exist?")
    sh_import_with_lot_serial_boolean = fields.Boolean(
        "Import Without Lot-Serial Boolean", default=check_sh_import_with_lot_serial)
    sh_company_id_with_lot_serial = fields.Many2one(
        related="base_id.sh_company_id_with_lot_serial", readonly=False, string="Company", required=True)
    ...
    def perform_the_action(self, record):
        ...
        if record.base_id.sh_technical_name == 'sh_import_inventory_with_lot_serial':
            self.import_with_lot_serial(record)
        return res

    def import_with_lot_serial(self,record):
        self = record
        stock_production_lot_obj = self.env['stock.lot']
        # # perform import Inventory
        if self and self.sh_file and self.sh_import_inventory_with_lot_serial_location_id and self.sh_import_inventory_with_lot_serial_name:

            if self.sh_import_inventory_with_lot_serial_type == 'csv' or self.sh_import_inventory_with_lot_serial_type == 'excel':
                counter = 1
                skipped_line_no = {}
                count=0
                try:
                    if self.sh_import_inventory_with_lot_serial_type == 'csv':
                    ...
                    elif self.sh_import_inventory_with_lot_serial_type == 'excel':
                    ...
                    for row in range(self.count_start_from - 1, till):
                        try:
                            ...
                            if row[0] not in (None, ""):
                                field_nm = 'name'
                                if self.sh_import_inventory_with_lot_serial_product_by == 'name':
                                    field_nm = 'name'
                                elif self.sh_import_inventory_with_lot_serial_product_by == 'int_ref':
                                    field_nm = 'default_code'
                                elif self.sh_import_inventory_with_lot_serial_product_by == 'barcode':
                                    field_nm = 'barcode'
                                ...
                                if search_product and search_product.type == 'product' and search_product.tracking in ['serial', 'lot'] and row[3] != '':
                                    quant_id = self.env['stock.quant'].search([('location_id', '=', self.sh_import_inventory_with_lot_serial_location_id.id), (
                                        'company_id', '=', self.env.company.id), ('product_id', '=', search_product.id), ('lot_id', '=', row[3])], limit=1)
                                    quant_vals = {}
                                    if quant_id:
                                        quant_vals.update({
                                            'display_name': self.sh_import_inventory_with_lot_serial_name,
                                            'product_id': search_product.id,
                                            'location_id': self.sh_import_inventory_with_lot_serial_location_id.id
                                        })
                                        ...
                                        if row[3] not in (None, ""):
                                            ...
                                            elif self.sh_import_inventory_with_lot_serial_is_create_lot:
